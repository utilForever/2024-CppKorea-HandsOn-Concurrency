...
int main(int argc, char* argv[])
{
    ...
    std::vector<std::atomic<uint8_t>> pixels(bounds.first * bounds.second);
    ...
    for (size_t i = 0; i < bounds.second; ++i)
    {
        ...
        threads.emplace_back(
            [&, i, band_bounds, band_upper_left, band_lower_right]() {
                std::vector<uint8_t> band(band_bounds.first);
                render(band, band_bounds, band_upper_left, band_lower_right);

                // Write to the pixels vector using atomic operations
                for (size_t j = 0; j < band.size(); ++j)
                {
                    pixels[i * bounds.first + j].store(
                        band[j], std::memory_order_relaxed);
                }
            });
    }
    ...
    // Convert atomic vector back to regular vector if needed
    std::vector<uint8_t> final_pixels(pixels.size());

    for (size_t i = 0; i < pixels.size(); ++i)
    {
        final_pixels[i] = pixels[i].load(std::memory_order_relaxed);
    }

    write_image(argv[1], final_pixels, bounds);
